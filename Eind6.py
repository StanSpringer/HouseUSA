...
with tab1:
    ...
    fig.add_trace(go.Scatter(x=avg_price_per_year["yr_built"], y=avg_price_per_year["price"], mode='lines', name='Dataframe 1', line=dict(color='steelblue')))
    fig.add_trace(go.Scatter(x=avg_price_per_year2["yr_built"], y=avg_price_per_year2["price"], mode='lines', name='Dataframe 2', line=dict(color='lightblue')))
...
with tab1:
    ...
    #colorpaltete
    color_palet = {'Gerennoveerd':'steelblue', 'Niet Gerennoveerd':'lightblue'}
...
with tab2:
    ...
    df_cor = df[['price', 'bedrooms', 'bathrooms', 'sqft_living', 'sqft_lot', 'floors',
                'condition', 'sqft_above', 'sqft_basement']]
    df2_cor = df2[['price', 'bedrooms', 'bathrooms', 'sqft_living',
               'sqft_lot', 'floors', 'condition', 'grade',
               'sqft_above', 'sqft_basement', 'sqft_living15', 'sqft_lot15']]
...
with tab3:
    try:
        ...
        st.error("De 'statsmodels' module is niet geïnstalleerd. Voer 'pip install statsmodels' uit in je terminal en herstart de app.")
        st.stop()
...
#     # Bereken de OLS-regressie voor figuur 1
#     X1 = sm.add_constant(df[selected_feature])
#     y1 = df['price']
#     model1 = sm.OLS(y1, X1).fit()

#     # Bereken de OLS-regressie voor figuur 2
#     X2 = sm.add_constant(df2[selected_feature])
#     y2 = df2['price']
#     model2 = sm.OLS(y2, X2).fit()

#     # Haal de helling en interceptie van de modellen op
#     slope1 = model1.params[selected_feature]
#     intercept1 = model1.params['const']
#     slope2 = model2.params[selected_feature]
#     intercept2 = model2.params['const']

#     # Stel de kleur van de trendlijn in op rood (R) en pas de vergelijking toe
#     # fig1.update_traces(line=dict(color='red', equation=f'Y = {slope1:.2f}X + {intercept1:.2f}'))
#     # fig2.update_traces(line=dict(color='red', equation=f'Y = {slope2:.2f}X + {intercept2:.2f}'))
