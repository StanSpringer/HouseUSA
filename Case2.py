...
with tab1:
    ...
    legend_data = [{'name': 'Dataframe 1', 'marker': {'color': 'blue'}},
              {'name': 'Dataframe 2', 'marker': {'color': 'red'}}]
    ...
    for item in legend_data:
        st.write(f"Marker: {item['name']}", item['name'])
...
with tab2:
    ...
    fig1.update_traces(marker=dict(color='lightblue'))
    fig2.update_traces(marker=dict(color='steelblue'))

    # Stel de kleur van de trendlijn in op rood (R)
    fig1.update_traces(line=dict(color='steelblue'))
    fig2.update_traces(line=dict(color='lightblue'))
